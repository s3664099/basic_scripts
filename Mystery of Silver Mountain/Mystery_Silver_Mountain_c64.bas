...
500 IFVB<NVANDC(B)<>0THENLETR$="YOU DO NOT HAVE THE "+T$:GOTO30
...
570 GOTO 650
580 ONVB-13GOSUB1960,1980,2010,2050,2870,2120,2220,2310,2380,2420,2450,2470,2520
590 GOTO 650
600 ONVB-26GOSUB2550,2580,2610,2650,2670,2700,2720,2730,2830,2800,2870,2730,2920
610 GOTO 650
620 ONVB-39GOSUB2950,2990,3010,3050,3070,2310,2990,3070,3130,2120,3190,1470,3100
630 GOTO 650
...
900 IFC(8)=0AND((R=52 AND D=2)OR(R=31ANDD<>3))THENLETR$="THE BOAR IS TOO HEAVY":RET
910 IFC(8)<>0AND((R=52ANDD=4)OR(R=31ANDD=3))THENLETR$="YOU CANNOT SWIM":RETURN
...
1010 IFR=49ANDD=2ANDF(54)=0THENLETR$="MYSTERIOUS FORCES HOLD YOU BACK":RETURN
...
1040 IFR=58AND(D=1ORD=4)ANDF(66)=0THENLETR$="YOU GET CAUGHT IN THE WEBS!":RETURN
1050 IF R=48 AND D=4 AND F(70)=0 THEN LET R$="THE DOOR DOES NOT OPEN":RETURN
1060 IF R=40 AND F(47)=1 THEN LET F(68)=1
1070 IFR=37ANDD=4ANDE$(37)="EW"THENLETR=67:LETR$="THE PASSAGE WAS STEEP!":RETURN
...
1200 IFF(39)>5ANDF(29)=1THENLETR$="CPPUT IBWF XPSO PVU":GOSUB4260:LETF(29)=0:LETC(3)=81
1210 RETURN
1220 GOSUB 3330:LET R$="OK":LET F(49)=0
1230 PRINT "YOU HAVE ";
1240 FORI=1TOG:READO$:GOSUB3350:IFI=1ANDC(1)=0ANDF(44)=1THENLETO$="COIN"
...
1600 IF(H=1443ORH=1485)ANDF(33)=0THENLETR$="B HMJNNFSJOH GSPN UIF EFQUIT":GOSUB4260
...
3420 LET B$="NOOEOOSOOWOOUOODOOINVGETTAKEXAREAGIVSAYPICWEATIECLIRIGUSEOPE"
3430 LET B$=B$+"LIGFILPLAWATSWIEMPENTCROREMFEETURDIVDAILEATHRINSBLODROEATMOV"
3440 LET B$=B$+"INTRINCUTHOLBURPOISHOUNLWITDRICOUPAYMAKBRESTEGATREF"
3450 LET X6$="ZPV SFGMFDUFE UIF XJABSET HMBSF! IF JT EFBE"
3460 LET X1$="THE GHOST OF THE GOBLIN GUARDIAN"
3470 LET X2$="B MBSHF WJOF HSPXT JO TFDPOET!"
3480 LET X3$="A GRARG PATROL APPROACHES"
3490 LET X4$="MAGIC WORDS LIE AT THE CROSSROADS, THE FOUNTAIN AND THE "
3500 LET X5$="A PILE OF RUBBLE BLOCKS YOUR PATH"
3510 LET X7$="THE MOUNTAIN RUMBLES!"
3520 LET X8$="TOWERS FALL DOWN!"
3530 LET X9$="THE WIZARD HAS YOU IN HIS GLARE"
3540 LET XB$="HE LEADS YOU "
...
2670 DATA 31GRARG SENTRY POST,12GUARD ROOM
...
3870 DATA 11MOSAIC-FLOOR HALL,12SILVER THRONE ROOM
3880 DATA 12MIDDLE OF THE LAKE,42EDGE OF AN ICY LAKE
3890 DATA 41PITTED TRACK,31HIGH PINNACLE
3900 DATA 55ABOVE A GLACIER,21HUGE FALLEN OAK
3910 DATA 11TURRET ROOM WTH A SLOT MACHINE,11COBWEBBY ROOM
3920 DATA 31SAFE IN OGBAN'S CHAMBER,31CUPBAORD IN A CORNER
...
4000 DATA 62END OF A BRIDGE,62ENDOF A BRIDGE
4010 DATA 61CROSSROADS,41WINDING ROAD
4020 DATA 11VILLAGE OF RUSTIC HOUSES,11WHITE COTTAGE
4030 DATA 3COINS,1SHEET,3BOOTS,1HORSESHOE,2APPLES,1BUCKET,4AXE,1BOAT,1PHIAL
4040 DATA 3REEDS,1BONE,1SHIELD,3PLANKS,1ROPE,1RINGS,1JUG,1NET,1SWORD
4050 DATA 1SILVER PLAT,1UNIFORM,1KEY,3SEEDS,1LAMP,3BREAD,1BROOCH,3MATCHES
...
4130 DATA MISTY POOL,WELL BOTTOM,OLD KILM,MOUNTAIN HUT
...
4170 DATA NS,E,NSW,N,NEW,EW,W,S,NS,N
...
4370 IF W$="G" THEN LET F(56)=1:RETURN
...
4620 RETURN
4630 LET F(69)=R:GOSUB 4640:GOSUB 4760:PRINT "BYE...":STOP
4640 PRINT:PRINT "PLEASE ENTER FILE NAME":INPUT FL$
4650 RETURN
4660 REM READ DATA FILE
4670 REM
4680 PRINT "OK. SEARCH FOR ";FL$
4690 OPEN 1,1,0,FL$:PRINT "OK. LOADING"
4700 FOR I=1 TO 80:INPUT#1,E$(I):NEXT
4710 FOR I=1 TO G:INPUT#1,C(I):NEXT
4720 FOR I-1 TO 70:INPUT#1,F(I):NEXT
4730 INPUT#1,G$(I):INPUT#1,G$(2)
4740 CLOSE 1:RETURN
4750 REM SAVE DATA FILE
4760 REM
4770 OPEN 1,1,1,FL$:PRINT "OK. SAVING"
4780 FOR I=1 TO 80:PRINT#1,E$(I):NEXT
4790 FOR I=1 TO G:PRINT#1,C(I):NEXT
4800 FOR I=1 TO 70:PRINT#1,F(I):NEXT
